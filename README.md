...
| 分类 | 包含设置 |
| --- | --- |
| 广告与内容过滤 | 广告过滤、发帖过滤（等级 / 关键词 / AI 标题党）、分享净化 |
| 界面与外观 | 液态玻璃底栏、底部导航栏隐藏、实验性功能 |
| 浏览与下载 | 解除复制、网页（重定向 / DevTools / 日志）、视频下载 |
...
- **覆盖范围**：首页推荐流、概念/资讯信息流、社区 Tab、话题详情、频道、游戏评测、榜单、
  搜索结果等帖子列表。数据层挂在宿主公共出口 `BBSLinkListResultObj#getLinks()` 上，
  渲染层再按 `(ViewHolder, BBSLinkObj)` 方法签名兜底；
  收藏夹、草稿箱与「添加到收藏夹」选择页属于你自己的数据，不参与屏蔽
- 改动规则后需刷新列表或加载新分页生效，本地缓存已渲染的帖子会在滚动重新绑定时补拦
...
- **液态玻璃底栏**（开关切换即时生效，无需重启；Android 13+ 为玻璃效果）：
  底部导航栏渲染为实时折射/色散的「液态玻璃」效果，带玻璃水滴选中动画、沉浸式小白条，
  标签/图标颜色随背景亮度自适应反色；关闭后立即恢复原生底栏，
  **长按首页标题栏右上角图标**或
...
- **浏览器重定向**（默认关闭）：内置网页中的链接按规则改由系统浏览器打开。
  ...
  「重定向外部链接」与「网页 DevTools」互斥，**打开重定向会同时把「包含小黑盒域名」复位为关**；
  内置网页容器按继承链识别（含小程序宿主），命中规则的页面在容器创建前就转出（无闪跳），
  已建容器的页面会连同容器一起关闭；容器关不掉的一律放行 ——
  不会出现「浏览器打开了、App 里页面空白」
- **包含小黑盒域名**：已知小黑盒域名也重定向，敏感页仍强制内置。默认关闭，开启重定向后
  确实需要再单独打开——官方网页 / 小程序（如「赛事中心」）靠内置 WebView 注入登录态，
  转外部会丢盒币等登录信息
- **官方 H5 / 小程序页面永远留内置**：`web.xiaoheihe.cn`（debug 包 `web.debugmode.cn`）
  下的页面与小程序首屏不参与重定向，开不开「包含小黑盒域名」都一样；确实要外开，
  把该域名加进「强制重定向域名」即可（显式配置优先）
...
- **分享渠道可配置**：设置面板「分享渠道」可选 **QQ / QQ空间**、**微信 / 朋友圈** 或 **微博**，
  ...
  渠道未出现在面板时自动改用面板里可用的渠道完成本步，且**只点分享面板渠道区内的按钮**——
  各渠道（QQ / QQ空间 / 微信 / 朋友圈 / 微博）的成功回调都在 SDK 入口被接管，
  不会真的把内容发出去，也不会触发 QQ / 微信客户端的下载引导页
- **链接要能打开原生页面**：配置的链接由小黑盒 RouterActivity 打开。若某条链接落到网页中转页
  （例如 `https://api.xiaoheihe.cn/heybox/game_statistic/`，它会 302 到腾讯应用宝的「小黑盒」下载页），
  自动化会**拦下这类下载页 / 应用商店跳转并跳过该步**（单步最长等 15 秒），不会再把页面弹到下载页；
  遇到这种情况请按下面的「链接格式」换成真正的分享链接
...
| 游戏详情分享链接 | `https://api.xiaoheihe.cn/game/share_game_detail?appid=123456&game_type=pc` |

> 链接经小黑盒 RouterActivity 自动路由到对应帖子/游戏页（部分链接会先过一层 H5 中转页再跳回 App，
> 属正常路径）；未配置的类型自动跳过。  
> **获取方式**：在小黑盒 App 打开目标帖子 → 分享 → 复制链接，取分享链接或网页链接均可；
> 游戏页 / 游戏详情页 → 分享 → 复制链接同理，得到的就是上面 `game/share_game_detail?appid=…` 这种形式  
> ⚠️ **别用网页中转 / 统计类链接**：像 `https://api.xiaoheihe.cn/heybox/game_statistic/`
> 这类地址会 302 到腾讯应用宝的「小黑盒」下载页，点开等于把用户弹去下载（模块会拦下并跳过该步，
> 但这一步的分享任务也就完不成）。  

### 动态推送（#26）

> 一级页点「动态推送」进入该分类的二级页，按「总开关 / 监控目标 / 抓取范围 / 提醒方式 /
> 第三方推送 / 测试与调试」分组；一级页的入口行会显示当前目标数量与获取时间窗。

- **关注动态提醒**：设置要关注的作者（userid 或用户主页链接），打开小黑盒 / 收到推送时检查其新动态
- **关键词监控**：设置关键词（`regex:` 前缀为正则），命中新帖标题或正文即提醒；
  「关键词只匹配标题」开启后不再匹配正文
- **关注话题**：一行一个话题名（或 `话题id|话题名`）；「导入关注话题」优先读小黑盒的关注话题，
  该接口不可用时回落到**你最近浏览过的话题**（模块会顺手记下宿主请求里的 `topic_id`，再批量换回名字）；
  「搜索话题」可按关键词搜话题 / 标签，点一下即加入
- **关键词 / 话题拉流**：开启后按关键词搜索、按话题取最新帖（首次只登记基线，之后只推新增；
  每轮最多处理 5 个关键词，关键词为 `regex:` 时跳过——正则不能当搜索词）
- **推荐关键词**：拉取小黑盒热搜词 / 联想词，点一下加入监控关键词
- **获取时间窗 / 检查间隔**：时间窗可选 30 分钟 ~ 30 天，间隔可选 5 分钟 ~ 12 小时
- **三种提醒方式**：
  - **应用内横幅**：在小黑盒界面顶部弹出，点击直达帖子
  - **系统通知**：通知栏提醒，点击**在盒内**打开帖子（显式指向宿主自己的路由页 / 帖子详情页，
    不会再把帖子丢给系统浏览器）
  - **第三方推送**：转发到钉钉机器人 / WxPusher / AstrBot 机器人 / 自定义 webhook；
    设置面板「测试提醒」一键验证横幅 + 通知 + 推送
    - AstrBot：填 `http://主机:6199|群号|访问令牌`（后两段可省，也支持 `private:QQ号`），
      走 AstrBot 的 aiocqhttp（OneBot v11）适配器
  - 设置面板另有「测试提醒」与「调试：推送最近 3 条」，用于一键验收各条链路
- **检查时机**：打开小黑盒、宿主收到推送（搭便车，无需任何凭据）、信息流命中；默认最小间隔 10 分钟
- **取数复用宿主自身的 OkHttp**：`hkey/_time/nonce` 由宿主拦截器照常补齐，不自己实现签名；
  由于小黑盒的 R8 会重命名 OkHttp 公开类，客户端实例与请求方法均按**结构**识别而非类名
- **跨版本**：1.3.393 / 1.3.394 / 1.3.395 的目标类与接口路径一致（已核对 393 与 395 的 APK），
  各 Hook 目标均为候选列表，安装时会输出命中汇总日志
- 默认关闭；未配置监控目标时不发起任何请求

### 通用

- **版本前置检测**：检测小黑盒版本是否为受支持版本 `1.3.393` / `1.3.394` / `1.3.395`，不匹配时显示提示
- **首启免责声明**：首次打开小黑盒强制弹出免责声明，需「同意并继续」方可进入；不同意直接退出应用，同意后不再重复显示
- **关于**：设置面板新增「关于」分组，可查看模块版本号并一键打开 GitHub 仓库
...
- **混淆名跨版本候选**：混淆名逐版本会变的目标按候选列表倒序尝试
  （如设置页入口 `N1` / `L1` / `G1`，气泡广告 `s(h$i)` / `l(h$g)`），
  新版本改名时优先自动命中；候选全部落空才回退到生命周期 Hook 或输出 WARN 提示

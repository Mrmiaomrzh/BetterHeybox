...
- **评论区自由复制**（#32，默认开启）：长按评论弹出的菜单保持不变，点「复制」后多出二级菜单：
  `复制全部内容`（= 原来的整条复制）、`自由复制`、`复制 @昵称`。
  「自由复制」打开居中卡片**完整**显示评论原文（长评论未展开也给全文、保留 `[表情]` 标记），
  **长按即唤起系统文本选择**自由框选复制（拖动跟手，选择过程中不会误触滚动）；
  也支持模块**自绘手柄**（跟随「解除复制 → 自绘制文本选择」开关，与帖子正文同一套）；
  卡片高度随内容自适应（最多约 55% 屏高后内部滚动），
  并与手势小白条联动（卡片下方那条同样是页面底色）。宿主「已复制到剪贴板」的提示会被抑制，
  只有真正复制时才提示；关闭开关即恢复宿主原「复制」行为
